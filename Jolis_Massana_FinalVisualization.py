...
def general_data_preparation(mass_shootings, county_population):
    ...
    mass_shootings_states = mass_shootings.groupby(['State', 'State_Lon', 'State_Lat', 'Abbreviation', 'Month,Year', 'Year', 'FIPS', 'Region', 'Population']).size().reset_index(name='Total Shootings')
...
def Q1_line_chart_regions(mass_shootings_regions, region_selection, date_selection, color_region):
    ...
    background_lines_regions = alt.Chart(mass_shootings_regions).mark_line(opacity = 0.3).encode(
        ...
        color = color_region,
        tooltip = ['Total Shootings:Q','Region:N', 'Month,Year:T']
    ).properties(
        width = 700,
        height = 300,
...
def Q1_region_state_charts(mass_shootings_states, region_selection, state_selection, date_selection):
    ...
    base_choropleth = alt.Chart(USA_states).mark_geoshape().transform_lookup(
        ...
        width = 550,
        height = 250,
        title = alt.TitleParams(text = 'Interactive Map of U.S. States: Explore Mass Shooting Data (2014–2024)',fontSize = 18, color = 'black', fontWeight='bold')
    ...
    opacity_states = alt.condition(state_selection, alt.value(1), alt.value(0.3))
    ...
    for region_name, color in regions.items(): 
        ...
        background_lines_states = alt.Chart(region_data).mark_line(opacity = 0.3).transform_filter(region_selection).encode(
            alt.X('Month,Year:T', axis = alt.Axis(title = 'Date', format='%b %Y', labelAngle=-45, titleColor = 'black', labelColor = 'black', titleFontSize = 14, labelFontSize = 12)),
            alt.Y('Total Shootings:Q', axis = alt.Axis(titleColor = 'black', labelColor = 'black', titleFontSize = 14, labelFontSize = 12)),
            tooltip = ['Total Shootings:Q', 'State:N', 'Region:N', 'Month,Year:T'],
            color = color
        ).properties(
            width = 650,
            height = 300, 
            title = alt.TitleParams(text = 'Mass Shootings in U.S. States (2014-2024) in the Selected Region',fontSize = 18, color = 'black', fontWeight='bold') 
...
def first_and_third_question(mass_shootings_regions, mass_shootings_states, mass_shootings, county_population):
    ...
    mass_shootings_states = mass_shootings_states.groupby(['State', 'State_Lon', 'State_Lat', 'Abbreviation', 'Region', 'Month,Year', 'Year','FIPS'])['Total Shootings'].sum().reset_index()
    ...
    region_choropleth = alt.Chart(USA_states).transform_lookup(
        ...
        title = alt.TitleParams(text = 'State-by-State Overview of Mass Shootings (2014-2024) and Region Selection',fontSize = 18, color = 'black', fontWeight='bold'), 
        width = 550,
        height = 250
    ...
    total_mass_shootings = alt.Chart(mass_shootings).transform_aggregate(
        ...
        tooltip = [alt.Tooltip('count:Q', title = 'Total Mass Shootings'), alt.Tooltip('State:N', title = 'State')]
    )
    ...
    state_abbreviations = alt.Chart(mass_shootings_states).mark_text().encode(
        longitude = 'State_Lon:Q',
        latitude = 'State_Lat:Q',
        color = alt.value('white'),
        text = 'Abbreviation:N'
    ).project(type = 'albersUsa')

    final_state_map = final_state_choropleth + state_abbreviations
    ...
    for state_name in mass_shootings_states['State'].unique(): 
        ...
        highlighted_top3_counties = alt.Chart(mass_shootings_top3_counties).mark_line(point = True, size = 2).encode(
            ...
            tooltip = ['Total Shootings:Q', 'City Or County:N', 'State:N','Region:N', 'Month,Year:T']
        ).properties(
            width = 700,
            height = 300,
            title = alt.TitleParams(text = 'Top 3 counties by mass shootings in the selected state (2014-2024)', fontSize = 18, color = 'black', fontWeight='bold') 
    ...
    Q1_final_dashboard = alt.hconcat(choropleths_state_linechart, region_county_linecharts, spacing = 60)
...
def second_question_slopechart(mass_shootings_regions):
    ...
    Q2_slopecharts_final = alt.hconcat(Q2_slopecharts, legend).properties(title = alt.TitleParams(
                                                text = 'Mass shootings per 10M people, by county and with respect to 2014',
                                                fontSize = 18,
                                                fontWeight = 'bold',
                                                color = 'black'))
...
def main():
    ...
    choro_scatter, _, slopecharts = st.columns([1, 0.01, 1.5]) 
    with choro_scatter:
        st.markdown( # customization of slider width
            """
            <style>
